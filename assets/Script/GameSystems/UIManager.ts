...
import { ProgressCallback, ResLoader } from "../GameUtilitys/ResLoader";
import { UIShowTypes } from "./UIConfig";
...
export class UIManager extends BaseSystem {
    ...
    private UICache: { [UIId: number]: BaseUIKeeperView } = {};
    /** UI界面栈（{UIID + UIView + UIArgs}数组）*/
    private UIStack: UIInfo[] = [];
    /** UI待打开列表 */
    private UIOpenQueue: UIInfo[] = [];
    /** UI待关闭列表 */
    private UICloseQueue: BaseUIKeeperView[] = [];
    /** UI配置 */
    ...
    public replaceUIConf(conf: { [key: number]: UIConf }): void {
        this.UIConf = conf;
    }
     ...
     */
    private autoExecAnimation(uiView: BaseUIKeeperView, aniName: string, aniOverCallback: () => void) {
        // 省略 这里可以放动画播放
        aniOverCallback();
    }
    /**
     ...
     */
    private autoLoadRes(uiView: BaseUIKeeperView, completeCallback: () => void) {
        // 省略 这里可以做页面级的预加载
        completeCallback();
    }
    /** 生成唯一的资源占用key */
    ...
    /**
     * 当页面栈发生变化时，根据界面显示类型刷新显示,例如全屏页面下的页面是可以不渲染的
     */
    private updateUI() {
     ...
     * @param uiArgs 初始化参数
     */
    private getOrCreateUI(uiId: number, processCallback: ProgressCallback, completeCallback: (uiView: BaseUIKeeperView) => void, uiArgs: any): void {
        // 如果找到缓存对象，则直接返回
        let uiView: BaseUIKeeperView = this.UICache[uiId];
        if (uiView) {
        ...
        let resLoader = this.getArchitecture().GetUtility(ResLoader)
        resLoader.load(uiPath, cc.Prefab, (err: Error, prefab: cc.Prefab) => {
            // 检查加载资源错误
            if (err) {
            ...
            // 检查组件获取错误
            uiView = uiNode.getComponent(BaseUIKeeperView);
            if (null == uiView) {
     ...
     */
    private onUIOpen(uiId: number, uiView: BaseUIKeeperView, uiInfo: UIInfo, uiArgs: any) {
        if (null == uiView) {
            return;
        }
        // 激活界面
        ...
        // 添加到场景中
        let child = cc.find("Canvas/GameUIRoot");
    ...
    /** 打开界面并添加到界面栈中 */
    public open(uiId: number, uiArgs: any = null, progressCallback: ProgressCallback = null): void {
        let uiInfo: UIInfo = {
            uiId: uiId,
            uiArgs: uiArgs,
            uiView: null
        ...
        this.getOrCreateUI(uiId, progressCallback, (uiView: BaseUIKeeperView): void => {
            // 如果界面已经被关闭或创建失败
            if (uiInfo.isClose || null == uiView) {
                cc.log(`getOrCreateUI ${uiId} faile! close state : ${uiInfo.isClose} , uiView : ${uiView}`);
     ...
     */
    public close(closeUI?: BaseUIKeeperView) {
        ...
        let close = () => {
            ...
            if (preUIInfo && preUIInfo.uiView && this.isTopUI(preUIInfo.uiId)) {
                // 如果之前的界面弹到了最上方（异步过程,中间可能打开了其他界面）
            ...
            if (uiView.cache) {
                ...
                cc.log(`uiView cache id: ${uiInfo.uiId}`);
            } else {
                // uiView.releaseAssets();
                uiView.node.destroy();
                cc.log(`uiView destroy id : ${uiInfo.uiId}`);
            }
            this.autoExecNextUI();
        }
        // 执行关闭动画
    ...
    public getUI(uiId: number): BaseUIKeeperView {
    ...
    public getTopUI(): BaseUIKeeperView {
        if (this.UIStack.length > 0) {
