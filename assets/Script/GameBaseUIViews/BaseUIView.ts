import { IArchitecture, IController } from "../FrameworkDesign/FrameworkBase";
import { SimpleGame } from "../GameApp";
import { UIShowTypes } from "../GameSystems/UIConfig";
/**
 * UIView界面基础类
 * 
 * 1. 快速关闭与屏蔽点击的选项配置
...
@ccclass
export class BaseUIView extends cc.Component implements IController {
    /** 快速关闭 */
    @property quickClose: boolean = false;
    /** 屏蔽点击选项 在UIConf设置屏蔽点击 或者在uimgr里改成ui配置*/
    // @property preventTouch: boolean = true;
    /** 缓存选项 */
    @property cache: boolean = false;
    /** 界面显示类型 */
    ...
    /**
     * 当界面被置顶时回调，Open时并不会回调该函数,args会获取上一个关闭页面的返回值
     * @param preID 前一个ui
     * @param args 可变参数，
     */
